...
- **`@containerlab/clab-ui`** consumed as a normal npm dependency from the public npm registry
...
Before writing any new component, hook, utility, or UI pattern, check whether `@containerlab/clab-ui` already provides it.
...
grep -r "SomeConcept" frontend/node_modules/@containerlab/clab-ui --include="*.d.ts" -l
```
...
- `@containerlab/clab-ui` is pinned to an **exact** version (`"0.3.2"`, no `^`) —
...
## Rules
- **Never deep-import from clab-ui** — only import from `@containerlab/clab-ui` root or its declared sub-entries (e.g. `@containerlab/clab-ui/host`).
