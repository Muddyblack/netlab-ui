 ...
 * --drawio --drawio-args --interactive`), bridged over a PTY/WebSocket the
 * same way node shells are — that wizard is a full-screen terminal UI, so it
...
export function DrawioWizard({ sessionId, onClose }: { sessionId: string; onClose?: () => void }) {
  ...
  useEffect(() => {
    ...
    const ws = new WebSocket(`${getApiWsBase()}/api/lab/graph/drawio/interactive?sessionId=${encodeURIComponent(sessionId)}`);
    ...
    <Box sx={{ display: "flex", flexDirection: "column", height: "100%", bgcolor: "background.default" }}>
      ...
      >
        <Typography variant="subtitle2" sx={{ fontFamily: "monospace", fontSize: "0.8rem", color: "text.primary" }}>
          draw.io interactive layout — arrow keys/space to assign levels, Enter to confirm
        </Typography>
