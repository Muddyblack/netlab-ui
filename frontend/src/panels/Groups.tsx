...
export function GroupsPanel({ sessionId, onChanged }: GroupsPanelProps) {
  ...
  return (
    ...
    >
          ...
          <Box sx={{ minWidth: 0 }}>
            ...
            <Typography variant="caption" color="text.secondary">
              {groups.length === 0
                ...
                : `${groups.length} ${groups.length === 1 ? "group" : "groups"} · expand a card for details`}
            </Typography>
          </Box>
          <Button variant="contained" size="small" startIcon={<AddIcon />} onClick={openCreate} sx={{ flexShrink: 0 }}>
            New
          </Button>
        </Stack>
        <Collapse in={selectedNodeIds.length > 0}>
          <Alert severity="info" icon={false} sx={{ mt: 1.25, py: 0.25, "& .MuiAlert-message": { fontSize: "0.75rem" } }}>
            {selectedNodeIds.length} canvas {selectedNodeIds.length === 1 ? "node is" : "nodes are"} ready to add to a new group.
          </Alert>
        </Collapse>
      </Box>
      ...
      {listView === "empty" && (
        ...
        >
          ...
          <Typography variant="body2" sx={{ fontWeight: 600 }}>
            {groups.length ? "No groups match" : "No configuration groups yet"}
          </Typography>
          <Typography variant="caption" color="text.secondary" sx={{ display: "block", mt: 0.5 }}>
            {groups.length
              ? "Try a different search term."
              : "Group nodes to configure routing and services together."}
          </Typography>
          {!groups.length && (
            <Button size="small" sx={{ mt: 1.5 }} onClick={openCreate}>
      ...
      <Dialog open={deleteTarget !== null} onClose={() => !saving && setDeleteTarget(null)} maxWidth="xs" fullWidth>
        ...
        <DialogActions>
          ...
          <Button color="error" variant="contained" onClick={() => void deleteGroup()} disabled={saving}>
            Delete group
          </Button>
        </DialogActions>
      </Dialog>
