        ...
        {loading && !unit ? (
          ...
          <>
            ...
            <PanelAddSection
              ...
              onAdd={() =>
                mutate(setIncludes)([...includes, { template: includableUnits[0]?.name ?? "", count: 1 }])
              }
            >
              {includes.length === 0 ? (
                  ...
                  {includes.map((row, idx) => (
                    <Box key={idx} sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                      ...
                      <IconButton
                        size="small"
                        aria-label="Remove"
                        onClick={() => mutate(setIncludes)(includes.filter((_, i) => i !== idx))}
                      >
                  ...
                  {connections.map((row, idx) => (
                    <Box key={idx} sx={{ display: "flex", alignItems: "center", gap: 0.75 }}>
                      ...
                      <IconButton
                        ...
                        onClick={() => mutate(setConnections)(connections.filter((_, i) => i !== idx))}
                      >
                        <DeleteIcon sx={{ fontSize: 18 }} />
                      </IconButton>
                    </Box>
            ...
            {/* Ports (exposed connection points) */}
            <Box>
              <Typography
                variant="caption"
                sx={{ display: "block", fontWeight: 700, letterSpacing: 0.4, textTransform: "uppercase", color: "text.secondary" }}
              >
                Exposed ports
              </Typography>
              {ownNodes.length === 0 ? (
                    ...
                    {ownNodes.map((node) => {
                      ...
                      return (
                        <Chip
                          ...
                          onClick={() =>
                            mutate(setPorts)(on ? ports.filter((p) => p !== node) : [...ports, node])
                          }
                        />
                      );
                    })}
            ...
            {/* Features (modules) */}
            <PanelAddSection title="Features (modules)" onAdd={() => mutate(setModules)([...modules, ""])}>
              {modules.length === 0 ? (
                <Typography variant="caption" color="text.secondary">
                ...
                <DynamicList
                  items={modules}
                  onChange={mutate(setModules)}
                  placeholder="e.g. bgp, ospf, vlan"
                  hideAddButton
                />
              )}
            </PanelAddSection>
