...
function buildEndpointSections(status: RunningLabsStatus, files: LabFileEntry[]): ExplorerTreeNode[] {
  ...
  const undeployedCount = files.filter((file) => isUndeployedFile(file, status)).length;
...
function buildUndeployedLabNode(file: LabFileEntry): ExplorerTreeNode {
  return {
  ...
  return {
    getChildren(element?: ExplorerTreeNode): ExplorerTreeNode[] {
      ...
      if (element.id === "endpoint:local") {
        return buildEndpointSections(runningLabsStatusRef.current ?? {}, labFilesRef.current ?? []);
      }

      if (element.id === "endpoint-section:running:local") {
        const status = runningLabsStatusRef.current ?? {};
        const labFiles = labFilesRef.current ?? [];
        return Object.entries(status).map(([labKey, labInfo]) => buildRunningLabNode(labKey, labInfo, labFiles));
      }

      if (element.id === "endpoint-section:local:local") {
        const status = runningLabsStatusRef.current ?? {};
        const files = labFilesRef.current ?? [];
        return files.filter((file) => isUndeployedFile(file, status)).map(buildUndeployedLabNode);
      }
